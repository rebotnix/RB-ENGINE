...
async function decrypt(content) {
  ...
  if (index == encryptSignature) { // file seems encrypted cause we found encrypt index marker at the end of the file
    try {
      ...
      const decryptedBytes = Rbc.AES.decrypt(content.slice(0,-encryptSignature.length), key, { mode: Rbc.mode.ECB, padding: Rbc.pad.Pkcs7 });
      const decryptText = decryptedBytes.toString(Rbc.enc.Utf8);
      content = decryptText.match(/.{1,2}/g).map(byte => String.fromCodePoint(parseInt(byte, 16))).join('');
    } catch(ex) {
      console.log("Error decrypting content L:182:",ex);
      process.exit(1);
    }
  }
